<template>
  <div id="qualification-loan-apply">
...
<script>
  ...
  const footer = [
    {
      name: "资质出借申请",
      icon: "qualification-loan-apply-icon.png",
      iconActive: "qualification-loan-apply-icon-active.png"
    },
    {
      name: "资质出借记录",
      icon: "qualification-loan-record-icon.png",
      iconActive: "qualification-loan-record-icon-active.png"
    }
  ];
  ...
  export default {
    ...
    beforeRouteEnter(to, from, next) {
      ...
      next(vm => {
        if (from.params.path == "saveBtn") {
          ...
          if (from.path != "/qualificationLoan-details") {
            vm.index = 0;
            vm.searchVal = "";
            vm.filterStatus = "";
            vm.filterType = "";
    ...
    methods: {
      ...
      async getData() {
        await _getData("license/licenseList", {
