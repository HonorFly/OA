<template>
  <div id="use-chapter-details">
    <header-vue :type="type" :data="details"></header-vue>
    <div class="container">
      <div class="top">
        <h2 v-if="type == 'apply' || type == 'edit'">盖章申请</h2>
        <steps-vue v-else :stepData="details" title="的盖章申请"></steps-vue>
  ...
  export default {
    data() {
      return {
        ...
        scrollTop: 0,
        submitFlag: false,
        isShowfooterApproval: false, //判断审批按钮是否显示
    ...
    methods: {
      ...
      submit(val) {
        ...
        _getData("seal/saveSeal", this.details).then(data => {
          console.log("提交：", data);
          this.submitFlag = true;
          this.$toast("提交成功");
      ...
      async getDetailInfo() {
        await _getData("seal/getSeal", {
          id: this.$route.query.id
        }).then(data => {
          console.log("盖章详情：", data);
          if (data.type == "Borrow") {
          ...
          // data.billCheckList = data.financeCheckList;
          let checkState;
          ...
          this.details = data;
          this.dateValue = this.details.useTime
            ? this.details.useTime.split(" ")[0]
            : "";
          if (this.dateValue) {
