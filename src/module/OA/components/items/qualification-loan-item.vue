<template>
  <li class="qualification-item" @click="handleClick(data.id)">
    <div class="main">
      <h2>
        {{ data.fullName }} 提交的公司资质出借申请
        <span>{{ data.updateTime ? data.updateTime : data.addTime }}</span>
      </h2>
      <ul>
        <li><span>资质所属公司</span>{{ data.belongCompany }}</li>
        <li>
          <span>使用时间</span>
          {{ data.useTime ? data.useTime : "--" }} 至
          {{ data.useTime1 ? data.useTime1 : "--" }}
        </li>
        <li>
          <span>归还时间</span>
          {{ data.predictTime ? data.predictTime : "--" }}
        </li>
        <li><span>出借方式</span>{{ data.ifSend }}</li>
      </ul>
      <div><span></span>{{ data.statusName }}</div>
      <span>{{ data.licenseType || data.type }}</span>
...
<style lang="scss" scoped>
  ...
  li.qualification-item {
    ...
    .main {
      ...
      & > span {
        background-image: linear-gradient(45deg, #4176ff 0%, #76afff 100%);
