<template>
  <li class="fund-item" @click="handleClick(data.id)">
    <div class="main">
      <h2>
        {{ data.fullName }} 提交的{{
          !Number(data.type) ? "个人借款" : "对公付款"
        }}审批<span>{{ data.applyTime }}</span>
      </h2>
      <ul>
        <li>
          <span>申请总额</span>{{ data.money ? data.money + "元" : "--" }}
        </li>
        <li><span>付款方式</span>{{ data.payType }}</li>
        <li>
          <span>项目名称</span><i>{{ data.itemName ? data.itemName : "--" }}</i>
        </li>
      </ul>
      <div><span></span>{{ data.statusName }}</div>
      <span>{{ !Number(data.type) ? "个人借款" : "对公付款" }}</span>
      ...
      handleClick(id) {
        ...
        this.$router.push({
          ...
          query: {
            ...
            fundName: !this.data.type ? "个人借款" : "对公付款"
          }
        });
      }
    }
